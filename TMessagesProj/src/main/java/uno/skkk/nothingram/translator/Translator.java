...
import uno.skkk.nothingram.helpers.PopupHelper;
import uno.skkk.nothingram.settings.NekoLanguagesSelectActivity;
...
public class Translator {
    ...
    public static void showTranslatorTypeSelector(Context context, View view, Runnable callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        arrayList.add(LocaleController.getString(R.string.TranslatorTypeService));
        types.add(NekoConfig.TRANS_TYPE_SERVICE);
    ...
    public static void showTranslationProviderSelector(Context context, View view, MessagesStorage.BooleanCallback callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        var selectedProvider = getCurrentProvider();
        PopupHelper.show(names, LocaleController.getString(R.string.TranslationProvider), types.indexOf(selectedProvider), context, view, i -> {
            ...
            if (translator.supportLanguage(targetLanguage)) {
                setCurrentProvider(types.get(i));
                if (callback != null) callback.run(true);
            } else {
                AlertDialog.Builder builder = new AlertDialog.Builder(context, resourcesProvider)
    ...
    private static String getCurrentProvider() {
        return NekoConfig.transType == NekoConfig.TRANS_TYPE_SERVICE ? NekoConfig.serviceTranslationProvider : NekoConfig.translationProvider;
    }

    private static void setCurrentProvider(String provider) {
        if (NekoConfig.transType == NekoConfig.TRANS_TYPE_SERVICE) {
            NekoConfig.setServiceTranslationProvider(provider);
        } else {
            NekoConfig.setTranslationProvider(provider);
        }
    }

    private static ITranslator getCurrentTranslator() {
        return getTranslator(getCurrentProvider());
    }
    ...
    private static ITranslator getTranslator(String type) {
        ...
        if (NekoConfig.transType == NekoConfig.TRANS_TYPE_SERVICE) {
            return ServiceTranslator.of(type);
        }
    ...
    private record TranslateTask(ITranslator translator, TLRPC.TL_textWithEntities query,
        ...
        public TranslationResult call() throws Exception {
            var key = Pair.create(query.text, tl + "|" + getCurrentProvider());
