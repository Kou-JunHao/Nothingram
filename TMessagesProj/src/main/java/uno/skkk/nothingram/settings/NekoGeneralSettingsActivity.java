...
public class NekoGeneralSettingsActivity extends BaseNekoSettingsActivity {
    ...
    private CharSequence getTranslationProvider() {
        ...
        var currentProvider = NekoConfig.transType == NekoConfig.TRANS_TYPE_SERVICE ? NekoConfig.serviceTranslationProvider : NekoConfig.translationProvider;
        int index = types.indexOf(currentProvider);
        if (index < 0) {
            return "";
        } else {
    ...
    private CharSequence getTranslatorType() {
        return switch (NekoConfig.transType) {
            ...
            case NekoConfig.TRANS_TYPE_SERVICE -> LocaleController.getString(R.string.TranslatorTypeService);
